...
export type SubmitSecondaryStakeResponse = {
  success: true;
  stakeId: string;
};
...
export function useSubmitSecondaryStakeMutation() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const response = await fetch("/api/stake/secondary", {
        method: "POST",
      ...
      if (!response.ok) {
        ...
        throw new Error(error.message || "Failed to submit secondary stake");
      }

      return response.json() as Promise<SubmitSecondaryStakeResponse>;
    },
  });
}
